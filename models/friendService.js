...
async function friendsService(decoded) {
	...
	if (idQuery.rows[0]) {
		const friendsQuery = await db.query(
			`
			SELECT u.id_user, u.email, u.username, u.avatar, u.hide_email, u.last_seen, u.deleted
			FROM public.user AS u, public.friendship AS f
			WHERE u.id_user = f.id_user_2 AND f.id_user_1 = $1 AND f.valid = TRUE AND u.deleted = FALSE
